...
var calculadora= {
    ...
    //Metodo Init
    init: (function(){
        this.changeSizeButton(" .tecla");
        this.getOperation();
    }),

    //Eventos click y cambio de tamaño a botones

    changeSizeButton: function(selector){
        var n=document.querySelectorAll(selector);
        for (var i=0; i<n.length; i++){
            n[i].onmousedown=this.makeSmallEvent;
            n[i].onmouseup=this.backTosizeEvent;
        };

    },

    makeSmallEvent:function(event){
        calculadora.makeSmallBtn(event.target);
    },

    backTosizeEvent:function(event){
        calculadora.backTosizeBtn(event.target);
    },

    //Dar formato a Botones
    makeSmallBtn: function(elemento){
		var x = elemento.id;
	...
	backTosizeBtn: function(elemento){
		var x = elemento.id;
		if (x=="1" || x=="2" || x=="3" || x=="0" || x=="igual" || x=="punto" ) {
	...
	//Asignacion de funciones a los eventos
	...
	updateVisor: function(){
		this.screen.innerHTML = this.valorVisor;
	}
